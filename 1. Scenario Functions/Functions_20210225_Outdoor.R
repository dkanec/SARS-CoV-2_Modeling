...
#OUTDOOR CODE AS OF MAY 25 2021
...
Aerofunc <- function(Event, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  facility.length <- 4.572
  facility.width <- 1.524
  facility.height <- 2.4384
  ...
  #Pre-VOC - NV
  #log.C.virus <- mcstoc(rtriang, type = "V", min = 5.396, mode = 8.092, max = 10.425)
  
  #Delta - NV
  #log.C.virus <- mcstoc(rtriang, type = "V", min = 5.642, mode = 7.618, max = 9.221)
  
  #Omicron - NV
  #log.C.virus <- mcstoc(rtriang, type = "V", min = 4.489, mode = 6.617, max = 8.505)
  
  #Delta - Vaxed - Breakthrough
  #log.C.virus <- mcstoc(rtriang, type = "V", min = 3.37, mode = 6.649, max = 8.778)
  
  #Omicron - Vaxed - Breakthrough 
  #log.C.virus <- mcstoc(rtriang, type = "V", min = 4.688, mode = 6.335, max = 8.233)
  
  #Original
  #Baseline.NV <- mcstoc(rtriang, type="V", min=6.1, mode=6.8, max=7.4)
  
  #Pre-VOC - NV
  #Pre.VOC.NV <- mcstoc(rtriang, type = "V", min = 5.396, mode = 8.092, max = 10.425)
  
  #Delta - NV
  #Delta.NV <- mcstoc(rtriang, type = "V", min = 5.642, mode = 7.618, max = 9.221)
  
  #Omicron - NV
  #Omicron.NV <- mcstoc(rtriang, type = "V", min = 4.489, mode = 6.617, max = 8.505)
  
  #Delta - Vaxed - Breakthrough
  #Delta.Vax <- mcstoc(rtriang, type = "V", min = 3.37, mode = 6.649, max = 8.778)
  
  #Omicron - Vaxed - Breakthrough 
  #Omicron.Vax <- mcstoc(rtriang, type = "V", min = 4.688, mode = 6.335, max = 8.233)
  
  #Baseline.Vax <- mcstoc(rtriang, type="V", min=5.7, mode=6.4, max=7.0)
  
  #Strain.VaxStat <- Strain.VaxStat
  
  #log.C.virus <- mcstoc(rtriang, type = "V", min = 0, mode = 0, max = 0)
  #ifelse(Strain.VaxStat == "Baseline.NV", log.C.virus <- Baseline.NV,
   #      ifelse(Strain.VaxStat == "Pre.VOC.NV", log.C.virus <- Pre.VOC.NV,
    #            ifelse(Strain.VaxStat == "Delta.NV", log.C.virus <- Delta.NV,
      #                 ifelse(Strain.VaxStat == "Omicron.NV", log.C.virus <- Omicron.NV,
        #                      ifelse(Strain.VaxStat == "Baseline.Vax", log.C.virus <- Baseline.Vax,
          #                           ifelse(Strain.VaxStat == "Delta.Vax", log.C.virus <- Delta.Vax,
          #                                    ifelse(Strain.VaxStat == "Omicron.Vax", log.C.virus <- Omicron.Vax, log.C.virus == 0)))))))
  ...
  #coughfreq <- mcstoc(runif, type="V", min=25, max=25)
  ...
  v.decay.lh <- mcstoc(runif, type = "VU", min = 11.111111, max = 12.5)  
  v.decay.hh <- mcstoc(runif, type = "VU", min = 11.111111, max = 12.5)  
  ...
  ###surface areas of stainless steel tables m^2; https://www.uline.com/BL_3853/Industrial-Packing-Tables
  ...
  fingers.sa <- mcstoc(rtriang, type = "V", min = 0.00093, mode = 0.00115, max = 0.00144)
  ...
  #inhalerate=mcstoc(runif, type="V", min=2.39, max=2.39)
...
AeroSensfunc <- function(Event, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  ###viral concentration in saliva, pfu/ml
  #Original
  #log.C.virus <- mcstoc(runif, type="V", min=6.8, max=6.8)
  
  log.C.virus <- mcstoc(rtriang, type="VU", min=6.1, mode=6.8, max=7.4)
  #log.C.virus <- mcstoc(rnorm, type = "V", mean = 6.77, sd = 0.264)
  #log.C.virus <- mcstoc(runif, type = "V", min = 6.1, max = 7.4)
  
  #Reduced Delta
  #log.C.virus <- mcstoc(rtriang, type="V", min=4.3, mode=5.83, max=6.83)
  #log.C.virus <- mcstoc(rnorm, type="V", mean = 5.65, sd = 0.518)
  #log.C.virus <- mcstoc(runif, type="V", min = 4.3, max = 6.83)
  
  #Reduced Delta (1log reduction)
  #log.C.virus <- mcstoc(rtriang, type="V", min=5.3, mode=6.83, max=7.83)
  #log.C.virus <- mcstoc(rnorm, type="V", mean = 6.65, sd = 0.517)
  #log.C.virus <- mcstoc(runif, type="V", min = 5.3, max = 7.83)
  
  #assuming delta totally infectious
  #log.C.virus <- mcstoc(rtriang, type="V", min=6.3, mode=7.83, max=8.83)
  #log.C.virus <- mcstoc(rnorm, type="V", mean = 7.65, sd = 0.517)
  #log.C.virus <- mcstoc(runif, type="V", min = 6.3, max = 8.83)
  ...
  ##total fraction of volume composed of droplet sizes 2-45 (ml/cough) --from paper reference above COUGH event
  v2.50 <- mcstoc(rtriang, type = "VU", min =0.0000013938 , mode =0.000002349, max =0.0000026395)
  
  ##total fraction of volume composed of droplet sizes <0.6 to 2.2 (ml/breathing) Papineni paper -- BREATHING event
  vbreath <- mcstoc(runif, type = "VU", min =0.000000000105743, max =0.000000000289581)
  ...
  #breathing rate: #breaths per minute per person range 16-20 breaths/min, Fabian et al., Origin of Exhaled Breath Particles from Healthy and Human Rhinovirus-Infected 
  # converted to breaths per hour per person; this range represents both high and low producers
  breathrate <- mcstoc(runif, type="VU", min=960, max=1200)
  
  #cough frequency: #coughs per hour per person
  coughfreq <- mcstoc(rtriang, type="VU", min=10, mode=25, max=39.25)
  #coughfreq <- mcstoc(runif, type="V", min=25, max=25)
  ...
  #fraction of pathogen transfer from hand to glove (4) during gloving process
  #f24<-mcstoc(runif,type="V",min=0,max=0.444)
  f24<-mcstoc(runif, type="VU", min=0, max=0.444)
  ...
  #Per https://dx.doi.org/10.1128%2FAEM.01030-13 Using MS2
  #Stainless Steel surface
  f12.hh <- mcstoc(rnorm, type = "VU", mean = 0.374, sd = 0.16)
  f12.lh <- mcstoc(rtriang, type = "VU", min = 0, mode=0.069, max= 0.158)
  ...
  #Updated based on Julian et al MS2 paper (same as above for transfer efficiencies)
  #NOTE: This was not RH dependent
  f23 <- mcstoc(rnorm, type = "VU", mean = 0.20, sd = 0.063)
  ...
  #updated viral inactivation rate on hands per Nicas et al (see below): based off of influenza A. Note: this is already in units min^-1
  #https://onlinelibrary-wiley-com.proxy.library.emory.edu/doi/full/10.1111/j.1539-6924.2009.01253.x
  v.decay.skin<- mcstoc(runif, type = "VU", min=0.92, max =1.47)
  v.decay.min <- v.decay.skin
  ...
  AeroSens <- mc(log.C.virus, v2.50, coughfreq, E.virus, vs, room.exchange, U.avg, v.us, Ct11, 
                 f12.hh, f23, v.decay.min.hand, hand.sa, fingers.sa, freq.hs, freq.hf, hw.eff, sc.eff.p, v.decay, fomite.decay,
                 DT.hh11, inhalerate, s.mask, s.mask.p, c.mask, oN95.mask, c.mask.p, oN95.mask.p, V.loss, u.star, Tw, Re, B, r.plus, D, Cc, 
                 Sc, A, I, Cf, dosetime11) 
...
DoseSensfunc <- function(Event, Volume.Fraction, Distance, Vol.Frac.Dist.Name, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  ##total fraction of volume of droplet sizes 50-60 ml/cough--for use in the droplet contact transmission COUGH event
  v50.60 <- mcstoc(rtriang, type = "VU", min = 0.00000351079, mode = 0.00000596834, max = 0.0000066705)
  
  ##total fraction of volume of droplet sizes 60-100 ml/cough--for use in the droplet contact transmission COUGH event
  v60.100 <- mcstoc(rtriang, type = "VU", min = 0.00000106218, mode = 0.00000491064, max = 0.00000841824)
  
  ##total fraction of volume of droplet sizes >100-750 ml/cough--for use in the droplet contact transmission COUGH event
  v100.750 <- mcstoc(rtriang, type = "VU", min = 0.003976453, mode = 0.006784165, max = 0.007646274)
  
  ##total fraction of volume composed of droplet sizes <0.6 to 2.2 (ml/breathing) Papineni paper -- BREATHING event
  vbreath <- mcstoc(runif, type = "VU", min =0.000000000105743, max =0.000000000289581)
  
  #Swtich to automatically select Volume Fraction
  VF <- mcstoc(rtriang, type = "VU", min = 0, max = 0, mode = 0)
  ifelse(Volume.Fraction == "50-60", VF <- v50.60, 
         ifelse(Volume.Fraction == "60-100", VF <- v60.100,
                ifelse(Volume.Fraction == "100+", VF <- v100.750, 
                       ifelse(Volume.Fraction == "breath", VF <- vbreath, VF == 0))))
  ...
  DropSens <- mc(log.C.virus, coughfreq, v50.60, v60.100, v100.750, s.mask, s.mask.p, E.virus, room.exchange, V.loss, Ct11, 
                 f12.hh, c.mask, oN95.mask, c.mask.p, oN95.mask.p, f23, v.decay.min.hand, hand.sa, fingers.sa, freq.hs, freq.hf, hw.eff, sc.eff.p, v.decay, fomite.decay, inhalerate, dosetime11)
