...
#TRANSPORTATION MODULE CODE AS OF MAY 25 2021
...
Aerofunc <- function(Event, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  #Hydraulic Diameter - in (meters) for TRANSIT BUS
  #facility.length <- 11.5
  #facility.width <- 2.6
  #facility.height <- 1.9
  
  #Hydraulic Diameter - in (meters) for INTERCITY BUS
  facility.length <- 12
  facility.width <- 2.55
  facility.height <- 2.66
  
  #Hydraulic Diameter - in (meters) for ARTICULATED BUS
  #facility.length <- 18
  #facility.width <- 2.55
  #facility.height <- 2.37
  
  #Inner volume of facility units is m3 - looks good
  facility.volume <- facility.length*facility.width*facility.height
  ...
  v.decay.lh <- mcstoc(runif, type = "VU", min = 0.150, max = 0.806)
  v.decay.hh <- mcstoc(runif, type = "VU", min = 0.150, max = 0.806)
  ...
  ### Assuming that for aerosols, the entire bus is the fomite, 
      #but for droplets only the surrounding seats would be the fomites
      #so 6m to their left and right, multiplied by the bus width
  sstable.sa <- facility.area
  ...
  #Updated this to PLASTIC for the bus seats
  #fomite.decay <- 0.148708621
  fomite.decay <- mcstoc(rtriang, type = "VU", min = 1/7.466667, mode = 1/(409/60), max = 1/6.166667)
  ...
  ### Changing this to represent PLASTIC transfer efficacies
  f12.hh <- mcstoc(rnorm, type = "VU", mean = 0.795, sd = 0.212)
  f12.lh <- mcstoc(rnorm, type = "VU", mean = 0.217, sd = 0.150)
...
Dosefunc <- function(Event, Volume.Fraction, Distance, Vol.Frac.Dist.Name, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  #NOTE: f12.lh is in a low humidity environment (15% - 32%)
  #Per https://dx.doi.org/10.1128%2FAEM.01030-13 Using MS2
  ### Changing this to represent polyester transfer efficacies
  f12.hh <- mcstoc(rnorm, type = "VU", mean = 0.023, sd = 0.008)
  f12.lh <- mcstoc(rnorm, type = "VU", mean = 0.217, sd = 0.150)
...
AeroSensfunc <- function(Event, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  AeroSens <- mc(log.C.virus, v2.50, coughfreq, E.virus, vs, room.exchange, U.avg, v.us, Ct1, f12.lh, f23, v.decay.min.hand, hand.sa, fingers.sa, freq.hs, freq.hf, hw.eff, sc.eff.p, v.decay, fomite.decay, DT.hh1, inhalerate, s.mask, s.mask.p, c.mask, oN95.mask, c.mask.p, oN95.mask.p, V.loss, u.star, Tw, Re, B, r.plus, D, Cc, Sc, A, I, Cf, dosetime1)
...
DoseSensfunc <- function(Event, Volume.Fraction, Distance, Vol.Frac.Dist.Name, room.exchange, Clean1, Clean2, Clean3, Clean4, Clean5, Clean6, Clean7, Clean8, Clean9, Clean10, Clean11, Clean12, sc.eff.p, Humidity, Infected.Mask, HW, Glove, Susceptible.Mask){
  ...
  DropSens <- mc(log.C.virus, coughfreq, v50.60, v60.100, v100.750, s.mask, s.mask.p, E.virus, room.exchange, V.loss, Ct1, f12.lh, c.mask, oN95.mask, c.mask.p, oN95.mask.p, f23, v.decay.min.hand, hand.sa, fingers.sa, freq.hs, freq.hf, hw.eff, sc.eff.p, v.decay, fomite.decay, inhalerate, dosetime1)
